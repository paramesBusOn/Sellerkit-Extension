...
Config config = Config();
  ...
  List<GlobalKey<SfCartesianChartState>> cartesianChartKey =
  ...
  initialMonthDataLoading() async {
    ...
    if (rangeType.contains('Month')) {
      ...
      dateRange[1].color = 1;
      String startDate2 =
          config.findFirstDateOfTheMonth2(DateTime.now()).toString();
      String endDate2 =
          config.findLastDateOfTheMonth2(DateTime.now()).toString();
            ...
            if (chartType![ik].viewCode == 'A103') {
              ...
              if (chartType![ik].chartType.toString() == 'Table') {
                ...
                tablercolumn.add({
                  ...
                  "PurchasePlan": getFilterData[i]['PurchasePlan'],
                  "NextFollowupDate":
  ...
  isselectRange(int i, String rangeType, BuildContext context,ThemeData theme) async {
    kpilottie = null;
    reportexp = null;
    leadAnalysismasterDataset2 = [];
  ...
  tablerColumn() {
    ...
    for (int i = 0; i < rangeFilterDBData.length; i++) {
      tablercolumn.add({
        ...
        "PurchasePlan": rangeFilterDBData[i].purchasePlan,
...
datePicker(
      BuildContext context, String rangeType, int i, ) async {
      ...
      builder: (context, child) {
        return Theme(
            ...
            colorScheme: ColorScheme.light(
              ...
              secondary: Color.fromARGB(255, 236, 155, 192),
                  onPrimary: Colors.white,
                  primary: Color(0xFF750537),
          ...
          child: child!,
          // data: themes.copyWith(
          //   scaffoldBackgroundColor: themes.primaryColor,
          //     appBarTheme: themes.appBarTheme.copyWith(
        ...
        if (assigResultTit[i]['Title'].toString() ==
          ...
          try {
            ...
            if (assigResultTit[i]['TestValues']
                ...
                .replaceAll(' ', '')
                .isAlphabetOnly) {
              // log('mmmmmm::::${assigResultTit[i]['TestValues'].toString()}');
              rangeTypee = 'Text';
              onTapstatus = false;
            } else if (assigResultTit[i]['TestValues'].toString().isNum) {
              onTapstatus = true;
              rangeTypee = 'Integer';
              // log('isNum::${assigResultTit[i]['TestValues'].toString()}');
            }
            // else {
            //   log('others::${assigResultTit[i]['TestValues'].toString()}');
            //   rangeTypee = 'Text';
            //   onTapstatus = false;
            // }
          }
