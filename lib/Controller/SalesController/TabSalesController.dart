...
import 'package:sellerkit/Pages/SalesInvoice/NewSales.dart';
...
class SalesTabController extends ChangeNotifier {
  ...
  itemselectinterest(String itemvalue, bool isselected) {
    ...
    log("selectedinterest::$selectedinterest");
    log("interestlevelvalue::$interestlevelvalue");
    notifyListeners();
  }
  ...
  itemselectassignto(String itemvalue, bool isselected) {
    ...
    log("selectedassignto::$selectedassignto");
    log("assignvalue::$assignvalue");
    notifyListeners();
  }
  ...
  itemselect(String itemvalue, bool isselected) {
    ...
    log("selectedlookingfor::$selectedlookingfor");
    log("lookingforvalue::$lookingforvalue");
    notifyListeners();
  }
  ...
  Future<void> dataget(List<Map<String, Object?>> assignDB, cusgropDB,
    ...
    for (int i = 0; i < cusgropDB.length; i++) {
      ...
      log("cusgroupcolumn::${cusgroupcolumn.length}");
      notifyListeners();
    }
  ...
  getdbmodel() async {
    ...
    log("GetfromdbEnqData::${GetfromdborderData.length}");
  ...
  callGetAllApi() async {
    ...
    await GetAllOrderApi.getData(ConstantValues.slpcode).then((value) {
      log("Step 2${value.stcode}");

      if (value.stcode! >= 200 && value.stcode! <= 210) {
        log("Step 3${value.Ordercheckdatageader}");
  ...
  mapValues(List<GetAllOrderData> leadcheckdata) async {
    log("inside method${leadcheckdata.length}");
    for (int i = 0; i < leadcheckdata.length; i++) {
      if (leadcheckdata[i].Status.toString().toLowerCase() == "open") {
        ...
        filterleadOpenAllData = leadOpenAllData;
        notifyListeners();
        log("isDelivered::${filterleadOpenAllData[0].isDelivered}");
        ...
        log("inside Open Docentry  ${leadcheckdata[i].Status}");
  ...
  mapvaluesmodify(GetAllOrderData? leadOpenAllData) {
    SalesNewController.datafrommodify.clear();
    SalesNewController.datafrommodify
        .add(leadOpenAllData!.OrderDocEntry.toString());

    SalesNewController.datafrommodify.add(leadOpenAllData.Mobile.toString());
    ...
    SalesNewController.datafrommodify.add(leadOpenAllData.bil_Area.toString());

    SalesNewController.datafrommodify.add(leadOpenAllData.bil_City.toString());
    ...
    SalesNewController.datafrommodify.add(leadOpenAllData.Del_Area.toString());

    SalesNewController.datafrommodify.add(leadOpenAllData.Del_City.toString());
    ...
    SalesNewController.datafrommodify.add(leadOpenAllData.OrderNum.toString());
    SalesNewController.datafrommodify.add(leadOpenAllData.gSTNo.toString());
    SalesNewController.datafrommodify
        .add(leadOpenAllData.CustomerGroup.toString());

    SalesNewController.datafrommodify
        .add(leadOpenAllData.OrderType.toString());
    SalesNewController.datafrommodify.add(leadOpenAllData.DocDate.toString());
    SalesBookNewState.iscomfromLead = true;
    ...
    if (filedata != null || filedata.isNotEmpty) {
      for (int i = 0; i < filedata.length; i++) {
        await OrderAttachmentApiApi.getData(
          ...
          log("OrderAttachmentApiApi::$value");
  ...
  callusermobileApi() async {
    await userbyidApi.getData(ConstantValues.UserId).then((value) {
      if (value.stcode! >= 200 && value.stcode! <= 210) {
        ...
        log("ConstantValues. userbyidmobile:::${ConstantValues.userbyidmobile}");
        getfirebase();
      }
    });
  }
...
//   }
  ...
  mapValuesToLead(BuildContext context) {
    SalesNewController.dataenq.add(leadDeatilsQTHData[0].CardCode!);
    SalesNewController.dataenq.add(leadDeatilsQTHData[0].CardName!);
    SalesNewController.dataenq.add(leadDeatilsQTHData[0].Address1!);
    SalesNewController.dataenq.add(leadDeatilsQTHData[0].Address2!);
    SalesNewController.dataenq.add(leadDeatilsQTHData[0].Pincode!);
    SalesNewController.dataenq.add(leadDeatilsQTHData[0].City!);
    Navigator.pop(context);
  ...
  checkValues() {
    print("getupdateFollowUpDialog: $getupdateFollowUpDialog");
    print("getleadForwarddialog: $getleadForwarddialog");
    print("getleadLoadingdialog: $getleadLoadingdialog");
    print("getforwardSuccessMsg: $getforwardSuccessMsg");
  }
}
